...
#		$validFiles=@("Table/Table_10.txt","Table/Table_14.txt","Codeunit/Codeunit_10.txt")
		...
		foreach ($objFl in $validFiles)
			{
				...
				$finSqlCommand = """$NAVFolder\finsql.exe"" command=compileobjects,file=$ExportFile,servername=$Server,database=$Database,synchronizeschemachanges=force,Logfile=$LogFile, navservername=TTRAFLOCO2K910, navserverinstance=DynamicsNAV80, navservermanagementport=7045, filter=Type=$objecttype;ID=$ID"
				Write-Debug $finSqlCommand
				cmd /c $finSqlCommand
				if (Test-Path $LogFile) 
					{   
		...
		$finSqlCommand = """$NAVFolder\finsql.exe"" command=exportobjects,file=$fobbuildfile,servername=$Server,database=$Database,Logfile=$LogFile, navservername=TTRAFLOCO2K910, navserverinstance=DynamicsNAV80, navservermanagementport=7045, filter=Version List='$versionlist C$LatestChSet'" 
		echo $finSqlCommand
		cmd /c $finSqlCommand
		if (Test-Path "$LogFolder\navcommandresult.txt")
			{
				...
				if (Test-Path "$fobbuildfile")
					{
						...
						Copy-Item -Path "${deployscript}" "${BldDrop}" -recurse -Force 
						#Copy-Item -Path "${BuildChngSetInfoFl}" "${BldDrop}" -recurse -Force 
