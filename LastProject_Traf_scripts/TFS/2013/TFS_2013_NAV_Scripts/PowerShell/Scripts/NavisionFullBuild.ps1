...
$SuccessVerFullFile="$BldChngSetFolder\SuccessVersionFull.txt"
$SuccessVerIncrFile="$BldChngSetFolder\SuccessVersionIncr.txt"
...
function MakeBuild()
{
	####Get the local workspace to version number of full changeset####
	echo "Getting the local workspace to version number of full changeset : $versionNumIncr"
	tf get /version:$versionNumIncr
...
#### End of Full Compilation  ####
	...
	$finSqlCommand = """$NAVFolder\finsql.exe"" command=exportobjects,file=$fobbuildfile,servername=$Server,database=$Database,Logfile=$LogFile, navservername=TTRAFLOCO2K910, navserverinstance=DynamicsNAV80, navservermanagementport=7045, filter=Version List='$versionlist'" 
	echo $finSqlCommand
	cmd /c $finSqlCommand
	if (Test-Path "$LogFolder\navcommandresult.txt")
		{
