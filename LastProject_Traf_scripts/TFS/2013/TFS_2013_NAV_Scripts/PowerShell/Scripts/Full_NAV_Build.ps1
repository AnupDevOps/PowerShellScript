...
New-Item -ItemType directory -Path $BldDrop\Master_Config_Package
New-Item -ItemType directory -Path $BldDrop\Automation_input_xml
################
# Fob logic here
################
function MakeBuild()
...
####  Full Compilation ######
				...
				$finSqlCommand = """$NAVFolder\finsql.exe"" command=compileobjects,servername=$Server,database=$Database,synchronizeschemachanges=force,Logfile=$FullLogFile, navservername=TTRAFLOCO2K910, navserverinstance=DynamicsNAV80, navservermanagementport=7045"
...
#### End of Full Compilation  ####
				...
				foreach ($objectFileName in $objArray)
						{
							...
							$finSqlCommand = """$NAVFolder\finsql.exe"" command=importobjects,file=$importFile,servername=$Server,database=$Database,synchronizeschemachanges=force,importaction=overwrite,Logfile=$LogFile, navservername=TTRAFLOCO2K910, navserverinstance=DynamicsNAV80, navservermanagementport=7045,filter=Type=$objecttype;ID=$ID"
							Write-Debug $finSqlCommand
							cmd /c $finsqlCommand
							if (Test-Path $LogFile)
								...
								{											
									...
									$finSqlCommand = """$NAVFolder\finsql.exe"" command=compileobjects,file=$ExportFile,servername=$Server,database=$Database,Logfile=$LogFile, navservername=TTRAFLOCO2K910, navserverinstance=DynamicsNAV80, navservermanagementport=7045,filter=Type=$objecttype;ID=$ID"
									Write-Debug $finSqlCommand
									cmd /c $finSqlCommand
									if (Test-Path $LogFile) 
										{   
	...
	$finSqlCommand = """$NAVFolder\finsql.exe"" command=exportobjects,file=$fobbuildfile,servername=$Server,database=$Database,Logfile=$LogFile,filter=Version List='$versionlist', navservername=TTRAFLOCO2K910, navserverinstance=DynamicsNAV80, navservermanagementport=7045" 
	echo $finSqlCommand
	cmd /c $finSqlCommand
	if (Test-Path "$LogFolder\navcommandresult.txt")
		{
			...
			if (Test-Path "$fobbuildfile")
					{
						...
						Copy-Item -path "${scriptPath}/../../Master_Config_Package/*"  -Destination "${BldDrop}/Master_Config_Package" -Container -force -recurse
						Copy-Item -path "${scriptPath}/../../Automation_input_xml/*"  -Destination "${BldDrop}/Automation_input_xml" -Container -force -recurse
						Copy-Item -path "${scriptPath}/automation_xml_input.txt"  -Destination "${BldDrop}/Automation_input_xml" -Container -force -recurse
