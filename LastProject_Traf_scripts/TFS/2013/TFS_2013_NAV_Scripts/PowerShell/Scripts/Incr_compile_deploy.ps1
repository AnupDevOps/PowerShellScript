		...
		if (!(Test-Path -Path $LogFolder\Error_*.txt))
			{
				...
				if (Test-Path -Path "${deployErrLogFile}" -PathType Leaf)
					{
						Remove-Item "${deployErrLogFile}" -Force
					}
				
#				Once the Build is success, start the Auto Deployment
				Invoke-Expression "C:\DEPLOY_SCRIPT\Direct_deploy.ps1"
#				Check for any error in the deployment of FOB file to target environment
				if (!(Test-Path -Path "${deployErrLogFile}" -PathType Leaf))
					{
						Write-Host "Deployment Successful" -ForegroundColor Green
						#After Successful Deployment, SucessVersion file is getting overwritten with the good changeset
						echo $LatestChSet >  $SuccessVerFile
					}
				else
					{
						Write-Host "Deployment Failed" -ForegroundColor Red
						Write-Host "Content of ${deployErrLogFile} is displayed below" -ForegroundColor Red
						cat "${deployErrLogFile}"
						exit 1
					}
			}
		else 
