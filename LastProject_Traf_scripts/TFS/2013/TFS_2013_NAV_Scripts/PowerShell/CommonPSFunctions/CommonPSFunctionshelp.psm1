...
SYNOPSIS
      ...
      Performs Check-out operation of Navision objects/files from Team Foundation Server
...
Write-Host "
      Example: ns co t 3
               Object of type=table with ID=3 will checkout of TFS with latest version.It will also update NAV DB with latest update.

      Example: ns co t `"3|10|1`” 
               Object of type=table with ID's=3,10,1 will checkout of TFS with latest version.It will also update NAV DB with latest update.`n`n" -ForegroundColor Green
}
...
SYNOPSIS
      ...
      Performs Check-in operation of Navision objects/files into Team Foundation Server
...
Write-Host "    
      Example: ns ci t 3
               Object of type=table with ID=3 will get ready to be checked-in into TFS with latest changes.
               A Check-in window will pop-up with preselected objects to check-in. Assoicate them with TFS work-Item and perform checkin.

      Example: ns ci t `"3|10|1`”
               Object of type=table with ID's=3,10,1 will get ready to be checked-in into TFS with latest changes.
               A Check-in window will pop-up with preselected objects to check-in. Assoicate them with TFS work-Item and perform checkin.

      Note:   Only Compiled objects will be checked in into TFS.
...
SYNOPSIS
      ...
      Performs Check-out operation of Navision objects/files from Team Foundation Server based on the object ids mentioned in upgradeConfig.ps1 file for a particular object type
...
Write-Host "
      Example: ns uco t
               Object of type=table with IDs as mentioned in upgradeConfig.ps1 file will checkout of TFS with latest version.It will also update NAV DB with latest update.`n`n" -ForegroundColor Green
}
...
SYNOPSIS
      ...
      Performs Check-in operation of Navision objects/files into Team Foundation Server based on the object ids mentioned in upgradeConfig.ps1 file for a particular object type
...
Write-Host "
      Example: ns uci t
               Object of type=table with IDs as mentioned in upgradeConfig.ps1 file will get ready to be checked-in into TFS with latest changes.
              A Check-in window will pop-up with preselected objects to check-in. Assoicate them with TFS work-Item and perform checkin.
...
SYNOPSIS
      ...
      Performs Check-in operation of Navision objects/files into Team Foundation Server based on the defined object ids and object types as mentioned in combineConfig.ps1 file 
...
Write-Host "
      Example: ns ci combine
               Object of various types as defined in `$objSequence variable and defined object IDs in combineConfig.ps1 file will get ready to be checked-in into TFS with latest changes.
               A Check-in window will pop-up with preselected objects to check-in. Assoicate them with TFS work-Item and perform checkin.
...
SYNOPSIS
      ...
      Performs list operation of Navision checkout objects/files from Team Foundation Server on Screen
...
SYNOPSIS
      ...
      Performs get-latest(import) operation of Navision objects/files from Team Foundation Server into Navision.
...
Write-Host "	  
      Example: ns gl t 3
               This will get-latest object of type=table with ID=3 from TFS and sync Navision with latest code.
      
      
      Example: ns gl t `"3|10|1`”
               This will get-latest objects of type=table with ID=3,10,1 from TFS and sync Navision with latest code.
      ...
      Example: ns gl all
               using all will sync Nav db with all latest objects present in TFS.`n`n" -ForegroundColor Green
}
...
Write-Host "	  
      Example: ns undo t 3
               This will get-latest object of type=table with ID=3 from TFS and revert the object to last TFS checkin state.
      
      
      Example: ns undo t `"3|10|1`”
               This will get-latest object of type=table with ID=3,10,1 from TFS and revert the objects to last TFS checkin state.`n`n" -ForegroundColor Green
}
...
SYNOPSIS
      ...
      Performs initial baseline of Navision objects into Team Foundation Server.
...
Write-Host "	  
      Example: ns bl all
               It will start initial baseine for all objects from Navision into Team Foundation Server.
               All licensed objects from database will be exported,added and checked-in into TFS.`n`n" -ForegroundColor Green
}

###Changes done by Anup Kumar Mishra ####
...
Write-Host "	  
      Example: ns eco
	  Before modifying any external object the developer is required to Check Out the object first.
	  This will lock the objects on TFS and nobody else can check out the object until the user check it in again.`n`n" -ForegroundColor Green
}
...
Write-Host "	  
      Example: ns eci
	  After modifying any external object the developer is required to Check in the object on TFS.`n`n" -ForegroundColor Green
}
...
Write-Host "	  
      Example: ns eundo
	  After modifying any external object the developer want to rollback or he want to start again the object on TFS.
