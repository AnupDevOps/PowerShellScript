...
#### End of Full Compilation  ####
	...
	$finSqlCommand = """$NAVFolder\finsql.exe"" command=exportobjects,file=$fobbuildfile,servername=$Server,database=$Database,Logfile=$LogFile,filter=Version List='$Versionupdate', navservername=TTRAFLOCO2K910, navserverinstance=NAVDEMO, navservermanagementport=7049" 
	echo $finSqlCommand
	cmd /c $finSqlCommand
	if (Test-Path "$LogFolder\navcommandresult.txt")
		{
